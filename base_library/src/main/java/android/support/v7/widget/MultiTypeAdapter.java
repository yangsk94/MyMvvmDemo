...
public class MultiTypeAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    private MultiTypeObservable mObserver;
    ...
    public MultiTypeAdapter(@NonNull List<?> items, @NonNull TypePool pool) {
        ...
        this.items = items;
        this.typePool = pool;
        mObserver = new MultiTypeObservable();
        try {
            FieldUtils.INSTANCE.writeField(this, "mObservable", mObserver);
        } catch (Throwable ignored) {
            mObserver = null;
        }
    }

    void registerLayoutObserver(MultiTypeObservable.LayoutObserver observer) {
        if (null != mObserver) {
            mObserver.registerLayoutObserver(observer);
        }
    }

    void unregisterLayoutObserver() {
        if (null != mObserver) {
            mObserver.registerLayoutObserver(null);
        }
    }
