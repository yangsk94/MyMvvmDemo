...
import static com.utils.reflect.ReflectUtils.print;
...
public class MethodUtils {
    ...
    private static Method getAccessibleMethod(Method method) {
        if (!MemberUtils.INSTANCE.isAccessible(method)) {
            return null;
        }
    ...
    private static Method getMatchingAccessibleMethod(final Class<?> cls,
        ...
        try {
            ...
            MemberUtils.INSTANCE.setAccessibleWorkaround(method);
            synchronized (sMethodCache) {
        ...
        for (final Method method : methods) {
            ...
            if (method.getName().equals(methodName) && MemberUtils.INSTANCE.isAssignable(parameterTypes, method.getParameterTypes(), true)) {
                // get accessible version of method
                final Method accessibleMethod = getAccessibleMethod(method);
                if (accessibleMethod != null && (bestMatch == null || MemberUtils.INSTANCE.compareParameterTypes(
        ...
        if (bestMatch != null) {
            MemberUtils.INSTANCE.setAccessibleWorkaround(bestMatch);
        }
        synchronized (sMethodCache) {
    ...
    public static <T> Constructor<T> getMatchingAccessibleConstructor(final Class<T> cls,
        ...
        Validate.INSTANCE.isTrue(cls != null, "class cannot be null");
        // see if we can find the constructor directly
        // most of the time this works and it's much faster
        try {
            final Constructor<T> ctor = cls. getConstructor(parameterTypes);
            MemberUtils.INSTANCE.setAccessibleWorkaround(ctor);
            return ctor;
        ...
        for (Constructor<?> ctor : ctors) {
            // compare parameters
            if (MemberUtils.INSTANCE.isAssignable(parameterTypes, ctor.getParameterTypes(), true)) {
                // get accessible version of constructor
                ctor = getAccessibleConstructor(ctor);
                if (ctor != null) {
                    MemberUtils.INSTANCE.setAccessibleWorkaround(ctor);
                    if (result == null
                            || MemberUtils.INSTANCE.compareParameterTypes(ctor.getParameterTypes(), result
    ...
    private static <T> Constructor<T> getAccessibleConstructor(final Constructor<T> ctor) {
        Validate.INSTANCE.isTrue(ctor != null, "constructor cannot be null");
        return MemberUtils.INSTANCE.isAccessible(ctor)
