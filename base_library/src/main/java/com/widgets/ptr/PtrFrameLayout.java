...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isDebug()) {
            Logger.INSTANCE.d(LOG_TAG, String.format("onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
        ...
        if (mContent != null) {
            ...
            if (isDebug()) {
                ...
                Logger.INSTANCE.d(LOG_TAG, String.format("onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin));
                Logger.INSTANCE.d(LOG_TAG, String.format("onMeasure, currentPos: %s, lastPos: %s, top: %s",
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (isDebug()) {
                Logger.INSTANCE.d(LOG_TAG, String.format("onLayout header: %s %s %s %s", left, top, right, bottom));
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (isDebug()) {
                Logger.INSTANCE.d(LOG_TAG, String.format("onLayout content: %s %s %s %s", left, top, right, bottom));
            }
        ...
        switch (action) {
            case MotionEvent.ACTION_UP:
                ...
                if (mPtrIndicator.hasLeftStartPosition()) {
                    if (DEBUG) {
                        Logger.INSTANCE.d(LOG_TAG, "call onRelease when user release");
                    }
                    onRelease(false);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    Logger.INSTANCE.v(LOG_TAG, String.format("ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown));
                }
    ...
    private void movePos(float deltaY) {
        ...
        if ((deltaY < 0 && mPtrIndicator.isInStartPosition())) {
            if (DEBUG) {
                Logger.INSTANCE.e(LOG_TAG, String.format("has reached the top"));
            }
            return;
        }
        ...
        if (mPtrIndicator.willOverTop(to)) {
            if (DEBUG) {
                Logger.INSTANCE.e(LOG_TAG, String.format("over top"));
            }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
                Logger.INSTANCE.i(LOG_TAG, String.format("PtrUIHandler: onUIRefreshPrepare, mFlag %s", mFlag));
            }
        }
        ...
        if (DEBUG) {
            Logger.INSTANCE.v(LOG_TAG, String.format("updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
    ...
    public void setRefreshCompleteHook(PtrUIHandlerHook hook) {
        ...
        hook.setResumeAction(new Runnable() {
            ...
            public void run() {
                if (DEBUG) {
                    Logger.INSTANCE.d(LOG_TAG, "mRefreshCompleteHook resume.");
                }
    ...
    private void performRefresh() {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            if (DEBUG) {
                Logger.INSTANCE.i(LOG_TAG, "PtrUIHandler: onUIRefreshBegin");
            }
        }
        if (mPtrHandler != null) {
    ...
    private boolean tryToNotifyReset() {
        if ((mStatus == PTR_STATUS_COMPLETE || mStatus == PTR_STATUS_PREPARE) && mPtrIndicator.isInStartPosition()) {
            if (mPtrUIHandlerHolder.hasHandler()) {
                ...
                if (DEBUG) {
                    Logger.INSTANCE.i(LOG_TAG, "PtrUIHandler: onUIReset");
                }
            }
    ...
    protected void onPtrScrollAbort() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, "call onRelease after scroll abort");
            }
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, "call onRelease after scroll finish");
            }
            onRelease(true);
        }
    }
    ...
    final public void refreshComplete() {
        if (DEBUG) {
            Logger.INSTANCE.i(LOG_TAG, "refreshComplete");
        }
        ...
        if (delay <= 0) {
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, "performRefreshComplete at once");
            }
            performRefreshComplete();
        } else {
            postDelayed(mPerformRefreshCompleteDelay, delay);
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, String.format("performRefreshComplete after delay: %s", delay));
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, String.format("performRefreshComplete do nothing, scrolling: %s, auto refresh: %s",
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mPtrIndicator.hasLeftStartPosition() && !ignoreHook && mRefreshCompleteHook != null) {
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, "notifyUIRefreshComplete mRefreshCompleteHook run.");
            }
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            if (DEBUG) {
                Logger.INSTANCE.i(LOG_TAG, "PtrUIHandler: onUIRefreshComplete");
            }
    ...
    private void sendCancelEvent() {
        if (DEBUG) {
            Logger.INSTANCE.d(LOG_TAG, "send cancel event");
        }
    ...
    private void sendDownEvent() {
        if (DEBUG) {
            Logger.INSTANCE.d(LOG_TAG, "send down event");
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        private void finish() {
            if (DEBUG) {
                Logger.INSTANCE.v(LOG_TAG, String.format("finish, currentPos:%s", mPtrIndicator.getCurrentPosY()));
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
                Logger.INSTANCE.d(LOG_TAG, String.format("tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance, to));
            }
            removeCallbacks(this);
